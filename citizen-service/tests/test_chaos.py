...
def test_new_faults_preserve_patch_semantics(client, monkeypatch):
    ...
    client.post("/api/chaos/fault", headers=headers, json={"latency_ms": 42})

    # Setting only cpu_burn must not disturb latency_ms or memory_leak_mb.
...
def test_reset_stops_cpu_burn_and_frees_leaked_memory(client, monkeypatch):
    monkeypatch.setattr(settings, "chaos_mode", True)
    monkeypatch.setattr(settings, "chaos_admin_token", "test-token")
    headers = {"X-Chaos-Token": "test-token"}
