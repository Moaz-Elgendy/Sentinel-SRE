...
class ChaosState:
    ...
    # the API model.
    #
    # IMPORTANT / deliberately not guarded against:
    # k8s/base/notification-service/deployment.yaml sets a 256Mi memory limit, so any value that pushes
...
class ChaosController:
    ...
    def set_default_notification_failure_rate(self, value: float) -> None:
        with self._lock:
            ...
            self._state = ChaosState(
                ...
                cpu_burn=self._state.cpu_burn,
                memory_leak_mb=self._state.memory_leak_mb,
            )
            self._publish()
    ...
    def reset(self) -> ChaosState:
        ...
        # chunk. (Note reset deliberately restores the *configured*
        # notification failure rate rather than 0.0 — that is this service's
        # long-standing behaviour, see CHAOS_FAILURE_RATE.)
        self._apply_side_effects(previous, new_state)
        return new_state
